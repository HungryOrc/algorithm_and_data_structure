...
public class Solution {
    ...
    public int findNthDigit(int n) {
        ...
        int numOfNumbersInCurDigit = 0;
        // 当前的位数里，一共包含多少位，比如1位数包含9位，2位数包含180位，3位数包含2700位……
        int totalDigitsOfNumbersInCurDigit = 0;
        // 给的数n，经过上述的逐位数的盘剥后，还剩多少
        int remainingN = n;
        
        // 如果remainingN > 0，意味着 n 还“有得剩”，意味着我们要找的数是大与或者等于当前位数的数
        while (remainingN > 0)
        {
            numOfNumbersInCurDigit = (int)Math.pow(10,curDigit) - (int)Math.pow(10,curDigit-1);
            totalDigitsOfNumbersInCurDigit = numOfNumbersInCurDigit * curDigit;
            remainingN -= totalDigitsOfNumbersInCurDigit;
            curDigit++;
        }
        // 如果remaining <= 0，意味着我们要找的数，比当前的位数低一位
        // 包括在低一位数的末尾，即remaining == 0的情况，比如999这样的数
        // 所以这时候就要把位数-1，回到低一位去，找我们要找的数
        curDigit --;
        // 把remaining复原成正数，我们基于此继续寻找
        remainingN += numOfNumbersInCurDigit * curDigit;
        
        // 这表示我们最终停止在哪个数上（下面的其实是最终数的前一个数）。这个数由两部分加和而成：
        int curNumber = 
            // 这一部分表示n能超越的最大的整个位数，比如9、99、999之类的
            (int)Math.pow(10,curDigit-1) - 1 + 
            // 这一部分表示n在当前的最后的位数里，又前进了几个数。比如在99的基础上再加3个数就是(100, 101,) 102
            remainingN / curDigit; 
        // 这表示我们经过了最后的一个整的数以后，还需要最后前进几位
        int remainingDigitsInCurNumber = remainingN % curDigit;
        
        // 如果最后需要前进 >0 位，那么我们真正最后停下的数，是下一个数
        ...
        int result = 0;
        // 如果最后需要前进 0 位，那么我们真正最后停下的数，就是当前数。而且我们要的就是它的最后一位
        if (remainingDigitsInCurNumber == 0)
